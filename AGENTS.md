...
> **⚠ 2026-08-24 更新：本節結論已作廢，不要再匯入這份 CSV。**
> 以**里**層級重驗後發現其中一筆偏差 682 公尺，而該地址佔全部資料 12.2%。
> 當時只驗到行政區層級，看不出區內部的錯誤。詳見七-16 D。
...
### 15. 少數地址決定整張圖：高影響地址與里名驗證（2026-08-22）

盤點 3,459 個地址的**列數分佈**後發現的事，它改變了「怎麼確保定位正確」的做法。
...
|---|---:|
| 前 10 大地址 | **63.2%** |
| 前 50 大地址 | 75.4%（其中 42 個是門牌型、佔 73.4%）|

更關鍵的是：**七-11 記載那兩個被 OSM 錯誤定位的地址，合計佔全部資料的 19.0%**：

| 地址 | 列數 | 佔比 |
|---|---:|---:|
| `高雄市鳳山區文福里建國路三段539號…` | 17,822 | 15.0% |
| `高雄市三民區本館里昌裕街1號…` | 4,504 | 3.8% |

**含意**：一個錯誤的高頻座標，會讓地圖上五分之一的點指向錯誤地點，而且看起來完全正常。
反過來說 —— **人工確認 10 筆是可行的，確認 3,459 筆不是**。驗證資源應該集中在這裡。

**做法**（`geocode_verify.py`）：批次結束後印出「高影響地址」區塊（`--top`，預設 10），
每筆列出座標、NLSC 反查結果、以及一條可直接點開的 Google Maps 連結供目視確認。

#### B. 里名驗證：同一區內定位到錯誤街廓的唯一免費訊號

原本的 NLSC 驗證只比對**縣市＋鄉鎮**。那擋得住七-11 那種跨行政區的 26 公里誤差，
但擋不住「同一個區裡定位到錯誤街廓」。

NLSC 的 `TownVillagePointQuery` 其實**連村里一起回傳**，而業者地址有 **20.8%
帶里名（涵蓋 51.4% 的列）** —— 比區細得多，且完全免費。

**但刻意不預設拒絕**：里界附近的建物反查到相鄰里是正常現象，硬性拒絕會誤殺正確結果、
白白損失覆蓋率。折衷做法：
- 一律取回並比對，不一致者**在報告中揭露**（含列數，讓人知道影響多大）；
- 高影響地址區塊會逐筆標註 `⚠里名不符`；
- 需要最保守判定時用 `--strict-village` 強制拒絕。

判定邏輯抽成純函式 `village_verdict()` → `ok` / `mismatch` / **`unknown`**。
`unknown`（地址沒寫里，或 NLSC 沒回里）**必須與 `ok` 分開**：本專案 79.2% 的地址沒有里名，
把「沒得比」算成「比對通過」，報告會顯示「全部驗過了」而實際上一大半沒驗到那一層 ——
那是對稽核者的誤導，比不驗更糟。

**里名擷取的邊界**：`village_of()` 只認「區/鄉/鎮之後緊接的里」，
故 `太麻里鄉`、`后里區`、`大里區`、`水里鄉`、`埔里鎮`、`里港鄉`、`八里區`
這些**行政區名含「里」**的情況都不會被誤判成里名。
實測 719 個含「里」字的地址中正確擷取 685 個（95.3%），未擷取的 34 個逐筆檢查
確認全是行政區名或造字（見下），**沒有漏判真里名的案例**。

#### C. 業者地址含 Unicode 私用區造字（必然定位失敗）

實測有 **5 個地址 / 12 列**含 Unicode 私用區（PUA）字元，例如
`高雄市鳥松區埔里大同路25-96號`（`0xEA53`）。這是台灣公部門系統的
EUDC 造字，**標準 Unicode 沒有這個字**，任何地理編碼服務的資料庫都不可能對得上。

另有 **3 個地址 / 11 列**用描述式寫法表達生僻字：
`高雄市鳥松區（水＋土）埔里神農路507號`、`高雄市橋頭區頂(土＋人＋口＋皿)田段2419-0000地號`。

合計約 23 列，量極小，但列在這裡是為了**避免有人日後花時間除錯「為什麼這個地址永遠查不到」**
—— 那不是程式問題，是字元本身不存在於任何標準字集。要處理只能人工對照後改寫地址。

### 16. Google + NLSC 實測結果，以及那份 116 筆推估表的作廢（2026-08-24）

使用者取得 Google 金鑰後跑了 50 個地址的試跑。結論是**定位路徑成立**，
但同時推翻了七-12「那份 116 筆可以匯入」的結論。

#### A. 試跑結果：49/50 採用，0 筆被行政區驗證擋下

| 項目 | 數字 |
|---|---:|
| 正向查詢 | 50 次（每月 10,000 免費） |
| 採用（雙重驗證通過） | 49 址 / **89,582 列（75.6%）** |
| 拒絕・行政區不符 | 0 |
| 查無結果 | 1 址 / 303 列 |
| 產出 cell_id 對應 | 3,073 筆 |

**唯一查無的是 `上網或使用VoWiFi`** —— Google 回 `ZERO_RESULTS`。
這正是七-14 B 節說的「本來就不是位置」，**查不到是正確行為**，佐證了那節的判斷。

#### B. 最關鍵的驗證：七-11 那兩個地址，Google 給對了

七-11 記載 OSM 把這兩個地址定位到錯誤行政區（偏差最大 26 公里）。它們合計佔全部資料 19.0%：

| 地址 | Google 座標 | NLSC 反查 | 判定 |
|---|---|---|---|
| 高雄市鳳山區**文福里**建國路三段539號 | 22.636692, 120.345436 | 鳳山區**文福里** | ✅ 里層級都相符 |
| 高雄市三民區**本館里**昌裕街1號 | 22.658114, 120.331586 | 三民區**本館里** | ✅ 里層級都相符 |

**這是這條路可不可行的關鍵測試，通過了。**

#### C. 兩份表交叉比對：中位數差 19 公尺

104 個 cell_id 同時出現在「OSM+雙重驗證」與「Google+NLSC」兩份表中 —— 兩者方法完全獨立。

- **中位數差距 19 公尺**、多數在 20 公尺內
- 這種獨立來源的收斂本身就是強力佐證

#### D. **但那份 116 筆推估表有一個高影響錯誤，不應再匯入**

差距最大的一筆是 **682 公尺**，而且它是第 3 大地址（**14,544 列 = 全部資料的 12.2%**）：

| 來源 | 座標 | NLSC 反查里名 | 地址寫的里 |
|---|---|---|---|
| Google | 22.641129, 120.343354 | **寶業里** ✅ | 寶業里 |
| OSM 推估 | 22.647259, 120.342944 | **本上里** ❌ | 寶業里 |

以**里**層級重驗整份 116 筆：相符 27 / **不符 16** / 無法判定 73（地址本身沒寫里名）。
不符的 16 筆中，10 筆就是上述 682 公尺誤差；另 6 筆是 `國隆里凱旋路316號`
—— 那筆經查是**業者地址的里名過時**而非定位錯誤（兩個獨立來源只差 37 公尺，
且 NLSC 在該點 1.1 公里內都找不到國隆里）。

**處置：Google 版產出後，`data/cell_towers_橋檢_top40_已驗證.csv` 作廢、不要匯入。**
`cell_towers` 是 `ON CONFLICT DO UPDATE`，後匯入者覆蓋先匯入者 —— 若在 Google 版之後
匯入這份，會把 12.2% 的資料換成偏差 682 公尺的座標。

**為什麼七-12 當時沒抓到**：那時的驗證只做到**行政區**層級（縣市＋鄉鎮），
而這個錯誤在三民區內部，區層級看不出來。**里層級驗證（七-15 B）是後來才加的，
而它上線後抓到的第一個真實錯誤就是這一筆。** —— 這也說明「驗證做到哪一層」
決定了你看得見哪一類錯誤。

#### E. 里名不符不等於定位錯誤：要再查一步

試跑報告標了 2 筆里名不符，逐一查證後是兩種完全不同的情況：

| 地址 | NLSC 反查 | 周邊探測 | 判定 |
|---|---|---|---|
| 屏東縣東港鎮**船頭里**沿海路55號 | 東津里 | 100m 外就是船頭里 | ✅ **里界效應**，無害 |
| 高雄市鳳山區**國隆里**凱旋路316號 | 國富里 | 1.1 公里內找不到國隆里 | ⚠ **業者地址里名過時**（Google 與 OSM 只差 37m）|

**查證手法（免費、不耗 Google 額度）**：拿 NLSC 反查在該座標周邊 ±100~250 公尺取樣。
若地址寫的里出現在附近 → 里界效應；若完全找不到 → 再擴大到 ±1 公里，
仍找不到就代表地址的里名有問題，而非座標有問題。

### 17. 全量 Google 跑完的結果，與**短碼 cell_id 全域衝突**（2026-08-24）

3,187 個地址全跑完。定位路徑成立，但同時讓五-X 預言的短碼問題**第一次實際發生**。

#### A. 結果：99.3% 涵蓋，驗證層擋下 118 個錯誤匹配

| 項目 | 數字 |
|---|---:|
| 正向查詢 | 3,187 次（免費額度 10,000 內）|
| 採用 | 3,065 址 / **117,753 列（99.3%）** |
| 拒絕・行政區不符 | **118 址 / 482 列** |
| 查無結果 | 4 址 / 325 列 |
| 產出 cell_id 對應 | 9,965 筆 |

**驗證層擋下的都是真錯**（若無驗證，這些會變成地圖上看起來正常的錯誤點位）：

- `高雄市仁武區水管路13號` → 實際落在**楠梓區**稔田里
- `台南市仁德區保安路１段19巷5號` → 實際落在**中西區**府前里
- `台東縣卑南鄉溫泉村龍泉路30之2號` → 實際落在**太麻里鄉**美和村
- `高雄市新興區民族二路33號` → 實際落在**苓雅區**光耀里

4 筆查無的全部是非位置字串：`上網或使用VoWiFi`、`WiFi熱點:VoWiFi`、
`路由:中華電信行動暨通信分公司`、`路由:中華電信公司` —— **查不到是正確行為**。

#### B. ⚠ 短碼 cell_id 在全域表中會對到不同基地台

產出的 9,965 筆裡，**400 個 cell_id 對到兩個以上不同座標**。依編號長度分組後，
分界線乾淨得沒有例外：

| cell_id 長度 | 數量 | 座標衝突 | 衝突率 | 衝突距離中位數 |
|---:|---:|---:|---:|---:|
| 1 | 9 | 8 | **88.9%** | 33,624 m |
| 2 | 73 | 50 | 68.5% | 6,808 m |
| 3 | 186 | 33 | 17.7% | 4,977 m |
| 4 | 1,041 | 61 | 5.9% | 4,639 m |
| 5 | 1,738 | 248 | 14.3% | 4,519 m |
| 8 / 9 / 15 / 20 | 6,308 | **0** | **0%** | — |

**最大衝突距離 306 公里**（例：cell_id `8722` 同時對到高雄前鎮與台北）。

**根因**：`cell_towers` 以 cell_id 為 UNIQUE 鍵、**跨案件共用一張全域表**；
但中華上網方言的「起址」是 1–5 碼短式編號，**只在單一業者的單一調閱案內有意義**
（五-X 已依編號形態推論過，此為首次實證）。加上 `ON CONFLICT DO UPDATE`
讓最後匯入者勝出 —— 某個點會拿到 300 公里外的座標，**而且在地圖上看起來完全正常**。

**處置**：`geocode_verify.py` 預設排除 `< 7` 碼的 cell_id（`is_globally_keyable()`，
可用 `--allow-short-cellid` 覆寫但不建議）。

**判準為什麼用長度而不是「本批有無衝突」**：沒衝突只代表**手上這批檔案**沒撞到。
若因此收進來，下一個案件匯入時才撞上，那時既有資料已被污染。
...
|---|---:|---:|
| 長碼可定位 | 109,935 | **93.2%** |
| 短碼・座標有衝突 | 2,122 | 1.8%（排除後不再誤導）|
| 短碼・本批無衝突 | 4,415 | 3.7%（保守起見一併排除）|

→ 排除短碼只損失 5.5% 涵蓋率，換掉 1.8% 會拿到隨機座標的列。

#### C. `village_of()` 的兩個相反方向的 bug（實測抓到）

全量跑的里名不符清單裡出現「地址寫 **區竹中里**」這種不存在的里名，順線查出兩個坑：

1. **沒有預查時**：`.{1,4}?` 非貪婪，「高雄市**前鎮區**竹中里」會先讓「鎮」滿足
   `[區鄉鎮市]`，區劃被切成「高雄市＋前＋鎮」→ 抓出「區竹中里」。全量資料中 19 筆中招。
2. **預查寫成 `(?![區鄉鎮市])`（第一版修法）**：「高雄市鳳山區**鎮北里**」因為里名以
   「鎮」開頭被迫回溯過頭，切成「鳳山區＋鎮」→ 抓出「北里」。**修好一個弄壞另一個。**

**最終解法只排除「區」**：`(?!區)`。因為**「區」不會是里名的第一個字，但「鎮/市/鄉」會**
（鎮北里、鎮南里、市中里都真實存在）。這個不對稱正是解開歧義的關鍵。

代價不是崩潰而是**假警報** —— 抓到不存在的里名 → 與反查必然不符 → 報告把正確的定位
標成可疑，讓人把注意力花在錯的地方；若有人用了 `--strict-village` 還會拒收正確結果。

#### D. 其他

- **金門座標正常**：4 筆 `金門縣金城鎮…` 落在 lng 118.29，在我先前用的「台灣本島框」
  （119.5–122.5）之外。那是**檢查框太窄**，不是資料錯 —— 外島是合法範圍。
  `api/cell_towers.py` 用的是 lat[-90,90]/lng[-180,180]，不受影響。
- 里名不符 110 址 / 1,325 列，**扣掉上述 19 筆假警報後**仍需人工判讀；
  判讀手法見七-16 E（用 NLSC 在周邊 ±100~250m 取樣，免費）。

### 18. 重複調用付費 API 的問題與修法（2026-08-24）

#### A. 問題：腳本繞過了所有既有快取

`geocode_verify.py` 的 `forward()` **直接呼叫 `_google_geocode`**，
繞過 `geocode.py` `lookup_bulk` 的四層快取（cell_towers → Redis → SQL → provider）。

實際代價：試跑 50 次之後跑全量 3,187 次，那 50 個被原封不動重查 —— **實際用掉 3,237 次**。
Google 每月免費額度 10,000 次，**沒有快取每月只跑得起 3 次**；日後新增案件檔，
等於把已查過的 3,187 個地址全部重查一遍。

#### B. 修法：檔案快取（`QueryCache`）

- 快取檔預設 `data/.geocode_cache.json`（`--cache` 可改、`--no-cache` 可停用）。
- **刻意用檔案而非 SQL `geocode_cache`**：本腳本設計成離線獨立執行
  （`DATABASE_URL` 可為假值），不該為了快取而綁上資料庫。
- **⚠ `.gitignore` 缺口**：`data/*.csv` 早已被全域 `*.csv` 規則涵蓋，但 **`.json` 沒有**。
  快取檔含案件地址，加快取時一併補上 `data/*.json` 與 `data/.geocode_cache*`。

**快取兩種查詢，用途不同**：

| key | 內容 | 為什麼快取 |
|---|---|---|
| `fwd\|{provider}\|{addr}` | 正向查詢結果 | **這是要錢的那個** |
| `rev\|{lat},{lng}` | NLSC 反查結果 | 免費，但 3,187 次 × 節流 ≈ 16 分鐘 |

key 帶 provider —— Google / TGOS / OSM 對同一地址的答案不同，混用會拿到別家的結果。

**三個刻意的決定**：

1. **負面結果也快取**。本專案有 4 個地址永遠查不到（`上網或使用VoWiFi`、
   `路由:中華電信公司` 等，它們本來就不是位置）。不快取的話每次重跑都為這些
   必然失敗的查詢再燒一次額度。要重試用 `--refresh-failed`。
2. **不快取「判定結果」，只快取原始查詢回應**。驗證邏輯本身還在演進
   （`village_verdict` 改過兩次、`village_of` 修過兩個相反方向的 bug），
   每次重跑都必須以最新規則重新判定 —— 否則舊的錯誤判定會被凍結在快取裡，
   永遠不會被發現。
3. **原子寫入 + 每 50 筆增量落地**。三千多址跑一小時，中途 Ctrl-C 不該白跑；
   而半截 JSON 檔的後果不是報錯而是**靜默失去整份快取**（下次讀取失敗 → 視為空白
   → 把三千多次查詢全部重打）。

#### C. 連帶修正：金鑰探測延後

原本在參數驗證階段就無條件探測 Google 金鑰。問題有兩個：
**全快取的重跑一次外部請求都不會發，此時金鑰有效與否無關緊要，卻會被擋下；
而且探測本身就燒一次額度。**

改成在「算出 targets 之後、進入長迴圈之前」才探測，且**只在有未快取項目時才做**。
仍保留「解析完檔案就失敗」的體驗（不會等完整個迴圈才發現金鑰壞掉）。

**實測**：拿一把**無效**金鑰跑全快取的 30 個地址 → 印出「（全部命中快取，不需要
Google 金鑰）」、`實際外部查詢 = 0`、結果照常產出。

#### D. 計數語意分家

`n_forward`（處理地址數）與 `api_calls`（**真實外部呼叫數**）分開回報，
且 `--max-requests` 護欄改看後者 —— 否則全快取重跑會在第 9,000 個地址停下來，
而實際花費是零。金鑰探測也計入 `api_calls`（它確實是一次真實呼叫）。

#### E. 實測效果

| | 快取 | 實際外部查詢 | 耗時 |
|---|---|---:|---:|
| 第 1 次（冷快取） | 命中 0 / 未命中 6 | **4** | 18 秒 |
| 第 2 次（暖快取） | 命中 6 / 未命中 0 | **0** | 0 秒 |
