...
| ~~P2.5-B~~ | ✅ **已完成** target 列表加「方位角」按鈕 → modal（select 三選一 + evidence ≥5字 textarea + 目前 by_ref 分佈摘要）→ 呼叫 PATCH → 成功 toast + reload。**真實 acceptance test 仍須在瀏覽器點按鈕實測。** | 偵查員可從 UI 完成法庭防禦性標註 | — |
| **P2.5-C** | 法庭防禦性 dashboard：顯示 unknown 比例 / 最近標註人 / audit_logs trail viewer / 報告 PDF 內顯示北方基準 | P2.5 整套 UI 閉環，可端對端應對律師詰問 | 半天 |
| ~~P2.6~~ | ✅ **已完成** popup 條件渲染整理：azimuth=null 時不再顯示「azimuth: °」空殼；cell_id 為空顯示「—」；azimuth=0（正北）邊界正確處理 | 細節體驗 | — |
