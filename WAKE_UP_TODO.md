...
> 更新時間：2026-08-24（Google 金鑰已通、試跑成功）　｜　程式碼對齊 commit `b32e726`+
>
...
2. **在本機跑**（不是在 production 跑）：
   ...
   - 想先小規模試水溫：加 `--limit 50`。
3. **先看報告最後的「高影響地址」區塊再匯入**（批次會自動印，預設前 10 筆）。
   本專案**前 10 大地址就涵蓋 63.2% 的列** —— 這幾筆的正確性幾乎決定整張圖，
   而報告會附 Google Maps 連結，逐筆點開目視確認只要幾分鐘。
   （七-11 那兩個被 OSM 錯誤定位的地址，合計就佔全部資料的 19.0%。）
   有 `⚠里名不符` 標記的要特別看 —— 那代表反查到的里跟地址寫的不一樣。
4. 確認無誤後：`admin.html` → 基地台座標表 → 匯入產出的 CSV。
5. production 維持 `GEO_GOOGLE_ENABLED=0`（**不要改**）—— runtime 永不呼叫 Google，
   費用不可能從線上復燃。
...
### ①-b ~~匯入那份 116 筆推估座標~~ ← **已作廢，不要匯入**

以**里**層級重驗後發現其中一筆偏差 **682 公尺**，而那個地址佔全部資料的 **12.2%**
（`寶業里陽明路170巷8號`：OSM 給的點反查是「本上里」，Google 給的點反查是「寶業里」）。

`cell_towers` 是 `ON CONFLICT DO UPDATE`，後匯入者覆蓋先匯入者 ——
若在 Google 版之後匯入這份，等於把 12.2% 的資料換成錯的座標。

**Google 版全面優於它**：104 個共同 cell_id 交叉比對，中位數只差 19 公尺，
而差距大的那幾筆都是 Google 對、OSM 錯。詳見 `CLAUDE.md` 七-16。
