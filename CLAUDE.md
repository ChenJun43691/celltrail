...
> 最近一次更新：2026-06-27（台哥大上網歷程格式 + 手動對應結構性修復 + geocode 並行/SQL 持久快取 + 雲端大檔上限；對齊 commit eeaef92）
...
2. **真表頭埋在 row 27**（前面是查詢條件 + 完整「使用者資料」PII 區塊：用戶名稱/帳寄地址/戶籍地址/證號/生日…），超過舊 `SCAN_WINDOW=25`。
   - 解法：`SCAN_WINDOW` 25→30。安全：PII/metadata 列 canonical 命中數=0，真表頭仍勝出，規則 B 不受影響。
...
| # | Task | 說明 |
|---|---|---|
| 0 | **後端串流式處理大檔（拆雲端記憶體牆）** | **P8 新增、優先**：上傳/parse-only 目前一次 hold 全部 record + GeoJSON 在記憶體，Render 小實例 >~5000 筆 OOM 502（見七-8）。改邊解析邊寫、回應只給摘要（或分頁/串流），讓大檔一次過。**目前用「分批 ≤5000 列」頂著**（使用者 2026-06-27 決定）。Claude 可做、中型改動。 |
...
### 8. 雲端單次上傳筆數上限 ≈ 5000–10000（記憶體 OOM）— **目前用分批頂著**

**事實**（2026-06-27 對 Render 線上實測，parse-only）：
- 5000 列切片 → ✅ 200 @30s、定位 4954/5000
- 完整 test3（21757 列 / 2423 唯一地址）→ 502；冷快取 @120s（逾時）、部分 SQL
  快取暖後 @**73.8s**（時間降證明 R 的快取生效，但 74s 就崩 = 記憶體 OOM）

**推論**：兩道牆 ① geocode 時間（R 的並行+SQL 快取已大幅改善、且可漸進收斂）
② **記憶體**：Render 小實例組裝「兩萬筆 record + 兩萬個 GeoJSON feature」在回應階段
OOM 被 kill → 502。快取救得了①，救不了②。

**目前作法（使用者 2026-06-27 拍板）**：**分批頂著** — 把大檔拆成 **≤5000 列**小檔
分批上傳（每份約 30s、定位 ~99%，且各自暖 SQL 快取）。21757 列拆 5 份即可。

**根治三選一（待辦，見六）**：① 後端改串流式（上傳不一次 hold 全部 record+GeoJSON，
回應只給摘要）→ 拆記憶體牆，**首選且 Claude 可做**；② 升級 Render RAM（花錢、最快）；
③ 填 `cell_towers`（消 geocode 時間牆，但記憶體牆仍在，需配①或②）。

> `GEO_GOOGLE_CONCURRENCY` env（預設 10）可在 Render 調高加速 geocode（仍受 Google
> ~50 QPS 配額）。注意 parse-only 限 **20 req/hr/IP**，盲測大檔很快用完配額。
...
backend/app/
  ...
  services/
    ingest.py        ← 核心邏輯（_normalize_row, _match_col_idx, _detect_dialect, _parse_ts,
                        ...
                        _read_xlsx_top_rows 假 dimension fallback, SCAN_WINDOW=30）
