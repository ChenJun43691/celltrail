...
pytest app/tests/ -v                                      # 全部（237 passed）
...
> **涵蓋範圍**（2026-05-26 已大幅補強，131 → 203；2026-05-30 → 216；2026-06-04 → 229；2026-06-06 → 234；2026-06-08 → 237）：
...
cd frontend/tests
npm install                                          # 一次性
npm test                                             # 不帶 token：公開頁 + 守衛重導向 + 訪客地圖互動 + 問答式對應（29 條）
# 帶 token 跑完整 46 條（另含 admin 三分頁、audit 查詢、登入後 UX 驗證、加密檔錯誤提醒）
...
| ~~6~~ | ~~前端 UI 自動化回歸~~ | **✅ 已有** — `frontend/tests/smoke.js`（playwright-core 驅動 Chrome，**46 條**）涵蓋公開頁/守衛重導向/admin 三分頁/audit 查詢/登入後 UX、**地圖互動**（訪客 parse-only 上傳→渲染 marker、popup 內容、測距工具、按鈕可見性）、**加密檔上傳錯誤提醒**、**問答式手動對應**（怪欄名→診斷→依範例值自動猜時間/地址欄）。走 parse-only 不寫 DB。`npm test`（帶 `CT_SMOKE_TOKEN` 跑完整 46 條）。 |
