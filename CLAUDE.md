...
**根本解**：填 `cell_towers`（待辦 #1）。手邊 16 檔共需 **6,620 個唯一 cell_id**、96.1% 的列帶 cell_id。此路一旦通，**速度與定位問題同時消失**，且完全不依賴任何外部 geocoding 服務。次選：重啟 Google（本批語料僅 2,240 個唯一地址，且 `geocode_cache` 持久化 → 一次性成本）。
