...
| ~~6~~ | ~~前端 UI 自動化回歸~~ | **✅ 已有** — `frontend/tests/smoke.js`（playwright-core 驅動 Chrome，32 條）涵蓋公開頁/守衛重導向/admin 三分頁/audit 查詢/登入後 UX。`npm test`（帶 `CT_SMOKE_TOKEN` 跑完整 32 條）。仍可視需要擴充地圖互動覆蓋。 |
