...
> 最近一次更新：2026-05-17（P3–P6 milestone 系列完成；本檔由 /init 重新對齊 git log）
...
# P3 後的兩個 migration（permission / account request；schema.sql 不含這兩張表）
...
### 7. P3 兩張表不在 schema.sql

`project_members` / `account_requests` 在 `migration_permissions.sql` / `migration_account_requests.sql`，新環境要另外套（見第三節）。
...
6. 看 `backend/app/db/schema.sql` + 兩個 migration 檔
