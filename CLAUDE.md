...
# 帶 token 跑完整 40 條（另含 admin 三分頁、audit 查詢、登入後 UX 驗證）
...
| ~~6~~ | ~~前端 UI 自動化回歸~~ | **✅ 已有** — `frontend/tests/smoke.js`（playwright-core 驅動 Chrome，**40 條**）涵蓋公開頁/守衛重導向/admin 三分頁/audit 查詢/登入後 UX，及**地圖互動**（2026-06-02 加：訪客 parse-only 上傳→渲染 marker、popup 內容、測距工具開關、按鈕群組可見性；走 parse-only 不寫 DB）。`npm test`（帶 `CT_SMOKE_TOKEN` 跑完整 40 條）。 |
